...
import 'package:e_commerce_clot/features/order/data/models/order_model.dart';
...
class OrderRepoImpl implements OrderRepo {
  ...
  Future<Either> getOrders()async {
    var returnedData = await getIt<OrderFirebaseService>().getOrders  ();
    return returnedData.fold(
      (error) {
        return Left(error);
      },
      (data) {
        return Right(
          List.from(
            data,
          ).map((e) => OrderModel.fromMap(e).toEntity()).toList(),
        );
      },
    );
  }
